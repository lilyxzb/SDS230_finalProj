...
First, we want to investigate whether there is a relationship between infant mortality and measles vaccination rates. We check the distribution of the variables and see that Measles vaccination rates are heavily left skewed and infant mortality is heavily right skewed. Importantly, the residuals have deviations form normality in the center and tails and we see evidence of heteroskedasticity and potenital outleirs in the fit vs studentized residual plot. We will likely need to perform some transformations ot these variables so that they are more symmetrical and uniformly distributed.
```{r}
# remove NAs 
...
```
=
There is a large left skew in measles vaccination. This makes sense, as the median is 93 and the mean is 87.21. Most of the data is centered on the right.  Since measles vaccination is a percentage, we can perform logit function on the measles explanatory variable. 
```{r}
